...
{
  NS_LOG_FUNCTION (this << tcb << segmentsAcked << rtt << m_ackedBytesTotal << m_ackedBytesEcn << tcb->m_ecnState << m_alpha );
...
{
  NS_LOG_FUNCTION (this << tcb << tcb->m_ecnState);
  if (m_ceState && m_delayedAckReserved && m_priorRcvNxtFlag)
    {
      SequenceNumber32 tmpRcvNxt;
      /* Save current NextRxSequence. */
...
TcpDctcp::CwndEvent (Ptr<TcpSocketState> tcb,
                     const TcpSocketState::TcpCAEvent_t event)
{
  NS_LOG_FUNCTION (this << tcb << event << tcb->m_ecnState << tcb->m_lastAckedSeq <<tcb->m_nextTxSequence);
  switch (event)
    {
